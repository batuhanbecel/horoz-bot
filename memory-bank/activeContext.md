...
- `cogs/utility/snipe.py` — `/snipe`
- `cogs/server/` log cogs — guild, member, message, voice logları
- `cogs/server/sayac.py` — `/sayaç kur`, `/sayaç kaldır`
- `cogs/fun/social.py` — `/anket`, `/etkinlik` (scheduled event + duyuru)
- `cogs/fun/games.py` — `/tkm`, `/adamasmaca`, `/8top`, `/kaccm`
- `cogs/fun/lol.py` — `/lol rehber` `buyu` `ipuclari` `counter` `eslesme` (38 build + Data Dragon)
- 10 yeni eğlence aracı: `/fikra`, `/atasozu`, `/trivia`, `/babasakasi`, `/qr`, `/sifre`, `/hava`, `/doviz`, `/altın`, `/deprem`
- `cogs/_v2.py` — Native discord.py 2.7.1 V2 components via native `to_component_dict()`. Accent color defaults removed from most cards (`color=None`); only `c_error`/`c_success` retain fixed colors.
...
- `/isimşehir`, `/vampirköylü`, `/rusruleti`, `/xox`, `/kelime başlat`, `/kelime bitir`, `/çekiliş`
...
### League of Legends
- `/lol rehber`, `/lol buyu`, `/lol ipuclari`, `/lol counter`, `/lol eslesme`
...
### Sunucu Araçları
- `/emoji-ekle`, `/oto-emoji`, `/sayaç kur`, `/sayaç kaldır`
