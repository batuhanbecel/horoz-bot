...
- `cogs/_v2.py` — Native discord.py 2.7.1 V2 components (`Container`, `TextDisplay`, `Section`, `Thumbnail`, `Separator`, `MediaGallery`). Raw dict builders replaced with native classes + `to_component_dict()` serialize. Accent color defaults removed from most cards (`color=None`); only `c_error`/`c_success` retain fixed colors.
